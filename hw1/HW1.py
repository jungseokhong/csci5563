...
def ConstructCylindricalCoord(Wc, Hc, K):
    ...
    #p = np.concatenate(((f*np.sin(ww*2*np.pi/Wc))[...,np.newaxis], (hh-Hc/2)[...,np.newaxis], (f*np.cos(ww*2*np.pi/Wc))[...,np.newaxis]), axis=2)
    p = np.zeros((Hc, Wc, 3))
    
    for h in range(Hc):
        for w in range(Wc):
            theta = 2*w*np.pi/Wc
            p[h][w][0] = f*np.sin(theta) 
            p[h][w][1] = h - Hc/2
            p[h][w][2] = f*np.cos(theta)
...
def Projection(p, K, R, W, H):
    ...
    for h in range(Hc):
        for w in range(Wc):
            p_t = R @ (np.array([p[h][w]]).reshape(3,1))
            u_homo = K@ p_t
            #u[h][w] = (u_homo/u_homo[2])[:2].reshape(2,)          
            u[h][w][0] = int(u_homo[0]/u_homo[2])
            u[h][w][1] = int(u_homo[1]/u_homo[2])
            if p_t[2] > 0 and u[h][w][0]>0 and u[h][w][0]<W and u[h][w][1]>0 and u[h][w][1]<H:
                mask[h][w]=1
...
if __name__ == '__main__':
    ...
    for i, (im_i, rot_i) in enumerate(zip(im_list, rot_list)):
        ...
        cv2.imwrite('test{}.jpg'.format(i+1),mask_2)        
        # Warp the image to the cylindrical canvas
        canvas_i = WarpImage2Canvas(im_i, u, mask_i)
        # Update the canvas with the new warped image
        canvas = UpdateCanvas(canvas, canvas_i, mask_i)
        #print("cavas shape", canvas.shape)
        cv2.imwrite('imagetest{}.jpg'.format(i+1),canvas)        
        #data = Image.fromarray(canvas.astype(np.uint8))
        #data.save('gfg_dummy_{}.png'.format(i+1)) 
        #plt.imshow(canvas)
